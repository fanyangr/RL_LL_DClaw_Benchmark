...
class RobotComponent(BaseComponent):
    ...
    def _perform_timestep(
        ...
        for config, control in group_controls:
            # assert len(control) == 12 # only supports dclaw with 3 Dof base
            # self.sim_scene.data.qpos[:3] = control[:3]
            # self.sim_scene.data.qvel[:3] = 0
        ...
        # print(self.sim_scene.data.ctrl[0])
        # self.sim_scene.data.ctrl[2] = 19
        # print(control[:3])
    ...
    def _apply_action_bounds(self, action: np.ndarray,
        ...
        "PID code"
        # cur_base_pos = self.sim_scene.data.qpos[config.qpos_indices][:3]
        # error = base_action - cur_base_pos
        # self.i_error += error
        # for i in range(len(self.i_error)):
        #     if self.i_error[i] > self.max_i_error[i]:
        #         self.i_error[i] = self.max_i_error[i]
        # d_error = error - self.prev_error
        # self.prev_error = error
        # base_action = self.kp * error + self.ki * self.i_error + self.kd * d_error
        "PID code end"
        ...
        # action[0] = z_axis_action # z axis action is force control, is not position control
        # action[:3] = base_action 
        # action[2] += 312
