...
class BaseDClawTurn(BaseDClawObjectEnv, metaclass=abc.ABCMeta):
    ...
    def get_obs_dict(self) -> Dict[str, np.ndarray]:
        ...
        output_claw_qpos = np.concatenate((np.zeros(3),claw_state.qpos))
        output_claw_qvel = np.concatenate((np.zeros(3), claw_state.qvel))
        output_object_info = np.concatenate((np.cos(object_state.qpos), 
                                            np.sin(object_state.qpos),
                                            ))
        ...
        # ))
        obs_dict = collections.OrderedDict((
            ('claw_qpos', output_claw_qpos),
            ('claw_qvel', output_claw_qvel),
            ('object', output_object_info),
            # ('last_action', self._get_last_action()),
            ('target_error', target_error),
        ))
    ...
    def get_reward_dict(
        ...
        claw_qpos = obs_dict['claw_qpos'][0, 3:]
        claw_vel = obs_dict['claw_qvel'][0, 3:]
        reward_dict = collections.OrderedDict((
            # Penalty for distance away from goal.
            ('target_dist_cost', -5 * target_dist),
            # Penalty for difference with nomimal pose.
            ('pose_diff_cost',
             -1 * np.linalg.norm(claw_qpos - self._desired_claw_pos)
            ),
    ...
    def get_score_dict(
        ...
        claw_qpos = obs_dict['claw_qpos'][:, 3:]
        claw_vel = obs_dict['claw_qvel'][:, 3:]
        target_dist = np.abs(obs_dict['target_error'])
