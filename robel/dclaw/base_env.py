...
class BaseDClawEnv(RobotEnv, metaclass=abc.ABCMeta):
    ...
    def __init__(self,
        ...
        self.model_name = kwargs['sim_model'].split('/')[-1]
    ...
    def _configure_robot(self, builder: RobotComponentBuilder):
        ...
        if 'fixed' in self.model_name and 'turn' in self.model_name:
            builder.add_group(
                'dclaw',
                ...
                qvel_range=[(-2 * PI / 2, 2 * PI / 2)] * 9)
        elif 'fixed' in self.model_name and 'grab' in self.model_name:
            builder.add_group(
                'dclaw',
                ...
                qvel_range=[(-0.2, 0.20)] + [(-2 * PI / 2, 2 * PI / 2)] * 9)
        elif 'tactile' in self.model_name and 'turn' in self.model_name:
            builder.add_group(
                'dclaw',
                qpos_indices=range(9),
                qpos_range=[
            ...
            builder.add_group(
                'dclaw',
                qpos_indices=range(12),
                qpos_range=[(-0.2, 0.2)]*3 + [
                    (-0.48, 0.48),  # ~27.5 degrees for top servos.
                    (-PI / 3, PI / 3),  # 60 degrees for middle servos.
                    (-PI / 2, PI / 2),  # 90 degrees for bottom servos.
                ] * 3,
                qvel_range=[(-0.20,0.2)]*2 + [(-0.2, 0.20)] + [(-2 * PI / 2, 2 * PI / 2)] * 9)
    ...
    def _initialize_action_space(self) -> gym.Space:
        ...
        if 'fixed' in self.model_name and 'turn' in self.model_name:
            return make_box_space(-1.0, 1.0, shape=(3 + qpos_indices.size,))
        elif 'fixed' in self.model_name and 'grab' in self.model_name:
            return make_box_space(-1.0, 1.0, shape=(2 + qpos_indices.size,))
        else:
...
class BaseDClawObjectEnv(BaseDClawEnv, metaclass=abc.ABCMeta):
    ...
    def __init__(self, *args, use_guide: bool = False, free_object=False, **kwargs):
        """Initializes the environment.
    ...
    def _configure_robot(self, builder: RobotComponentBuilder):
        ...
        if self.free_object:
             builder.add_group(
                'object',
                qpos_indices=range(-7,0),  # The object is the last qpos.
                qpos_range=[(-10, 10)]*3 + [(-1,1)] * 4,
                qvel_indices=range(-6,0),
                qvel_range=[(-10,10)] * 6 )
        else:
            builder.add_group(
                'object',
